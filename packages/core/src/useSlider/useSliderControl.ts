...
export function useSliderControl<TValue>(_props: Reactivify<SliderControlProps<TValue>, '_field' | 'schema'>) {
  ...
  function getThumbValue(idx: number) {
    if (Array.isArray(model.value)) {
      return mapStopToNumber(model.value[idx]);
    }

    if (idx === 0) {
      return mapStopToNumber(model.value);
    }
  ...
  function setThumbValue(idx: number, value: number) {
    ...
    if (thumbs.value.length <= 1) {
      setModelValue(mapNumberToStop(value));
      updateValidity();
      return;
    }

    const nextValue = normalizeArrayable(model.value).filter(v => !isNullOrUndefined(v));
    nextValue[idx] = mapNumberToStop(value);
    setModelValue(nextValue as TValue);
    updateValidity();
  }
